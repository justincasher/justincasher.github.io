...
&emsp; Suppose coproducts exist in $$ \cC $$, that every element in $$ \cC $$ is projective, and that the collection of isomorphism classes form a set, i.e. $$ \cC $$ modulo isomorphism is a set. (If we do not assume every object is projective, then we can define addition by $$ 0 \to A \to B \to C $$  implies $$ [B] = [A] + [C] $$. Then we can form a monoid with elements $$ [\Ob(\cC)] $$ and addition $$ [A] + [B] = [A \oplus B] $$, which is commutative since $$ A \oplus B \cong B \oplus A $$. We append inverses by as follows. 
